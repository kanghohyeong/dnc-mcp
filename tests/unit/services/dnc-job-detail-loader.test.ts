...
describe("DncJobDetailLoader", () => {
  ...
  describe("loadJobByIdWithDetails", () => {
    it("should load job by id from root job directory", async () => {
      // Arrange
      const dncDir = path.join(tempDir, ".dnc");
      const jobDir = path.join(dncDir, "job-find-me");
      await fs.mkdir(jobDir, { recursive: true });

      const job: DncJob = {
        id: "job-find-me",
        goal: "Find this job",
        spec: ".dnc/job-find-me/spec.md",
        status: "pending",
        divided_jobs: [],
      };
      ...
      // Act
      const result = await loader.loadJobByIdWithDetails("job-find-me");

      // Assert
      expect(result).not.toBeNull();
      expect(result?.id).toBe("job-find-me");
    ...
    it("should load job from nested divided_jobs", async () => {
      ...
      // Act
      const result = await loader.loadJobByIdWithDetails("job-nested-child");

      // Assert
      expect(result).not.toBeNull();
      expect(result?.id).toBe("job-nested-child");
      expect(result?.specContent).toBe("# Child");
    });

    it("should return null when job not found", async () => {
      // Act
      const result = await loader.loadJobByIdWithDetails("non-existent-job");
