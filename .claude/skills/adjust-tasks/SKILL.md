...
The system performs the actual modifications using specific DNC tools based on the task status:
  * **Task Modification (`modify`):** For tasks in the `modify` state, the system applies updates to goals or acceptance criteria according to the Custom Instructions using the `dnc-update-job` tool.
