...
The system identifies the target **Root Task**. If the target is ambiguous, it uses the `dnc-list-root-tasks` tool to list active projects and **requests user confirmation** to ensure the correct plan is being executed.
* **Step 2: Plan & Review Verification**
The system utilizes the `dnc-get-task-relations` tool to synchronize with the current task plan and review outcomes, ensuring all tasks are ready for deployment.
...
As work progresses, the system reflects real-time status changes using the `dnc-update-task` tool:
