...
* **Step 3: DNC Task Initialization** Once the goal is confirmed, the skill utilizes the `dnc-init-task` tool to formalize the **Root Task** within the system and assign a unique tracking ID.
...
* **Step 5: Hierarchical Sub-task Decomposition** Based on the selected depth, the system utilizes the `dnc-append-divided-task` tool recursively to build a **multi-layered task inheritance structure**. This process transforms the Root Task into a vertical tree where each sub-task is further decomposed until the leaf nodes represent immediate, granular actions ready for execution.